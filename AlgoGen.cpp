...
void AlgoGen::render(sf::RenderTarget& target)
{
	...
	/*updateTrajetsRetenus();

	for (int j = 0; j < NB_SOL; ++j)
	{
		for (int i = 0; i < nbToursSimules - 1; i++) // affichage du robot vert
		{
			sf::Vertex line[] =
			{
				sf::Vertex(sf::Vector2f(trajetsRetenus[j][i].getX(), trajetsRetenus[j][i].getY())),
				sf::Vertex(sf::Vector2f(trajetsRetenus[j][i+1].getX(), trajetsRetenus[j][i+1].getY()))
			};
...
void AlgoGen::genSol(const float& rightSpeed, const float& leftSpeed, const Point& target)
{
	...
	//test et evaluation de la nouvelle solution 
	{
		...
		for (int i = 0; i < nbToursSimules; i++)
		{
			...
			Point debug(m_posTest);

			actualisePositionTest(m_sol[NB_SOL][i][0] * TEMPS_GEN * 60 / 1000, m_sol[NB_SOL][i][1] * TEMPS_GEN * 60 / 1000, i);

			// debug
			if (dist(m_posTest - debug) > 100.f)
			{
				std::cout << "error" << m_sol[NB_SOL][i][0] << " " << m_sol[NB_SOL][i][1] << std::endl;

				m_posTest.Setall(debug.getX(), debug.getY(), debug.getangle());

				actualisePositionTest(m_sol[NB_SOL][i][0] * TEMPS_GEN * 60 / 1000, m_sol[NB_SOL][i][1] * TEMPS_GEN * 60 / 1000, i);

			}
			if (m_posTest.getX() < 0.f || m_posTest.getY() < 0.f)
			{
				std::cout << "error" << m_sol[NB_SOL][i][0] << " " << m_sol[NB_SOL][i][1] << std::endl;
...
void AlgoGen::actualisePositionTest(const float& rightSpeed, const float& leftSpeed, int indTour)
{
	...
	if (abs(rightSpeed - leftSpeed) > 0.00001f) {
...
// tests de fonctionnalités à ajouter
...
void AlgoGen::updateTrajetsRetenus()
{
	for (int j = 0; j < NB_SOL; ++j)
	{
		m_posTest.Setall(m_posDep.getX(), m_posDep.getY(), m_posDep.getangle());
		...
		for (int i = 0; i < nbToursSimules; i++) // affichage du robot vert
		{
			actualisePositionTest(m_sol[j][i][0] * TEMPS_GEN / 17, m_sol[j][i][1] * TEMPS_GEN / 17, i);
			trajetsRetenus[j][i].Setall(m_posTest.getX(), m_posTest.getY(), 0.f);
...
{
	return (std::string) "Score actuel : " + std::to_string((int)currentScore) + (std::string) "\nnombre de mutations : " + std::to_string((int)m_nbMutationsMax);
}
